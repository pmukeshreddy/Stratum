// Ported from supplied Prime core/refinement/refinement.ts:407-565.
// Only TypeScript annotations and product branding are changed.
...
function formatRefinementNoticeBody(result) {
...
function formatHarnessStateForPrompt(state, options = {}) {
...
function overviewForPrompt(state) {
	const lines = [];
	for (const kind of Object.keys(state.entries)) {
...
const [operation, indent] = process.argv.slice(2);
const result = operation === "format" ? formatHarnessStateForPrompt(value.state, value.options)
  : operation === "overview" ? overviewForPrompt(value)
  : operation === "history" ? historyForPrompt(value)
  : operation === "notice" ? formatRefinementNoticeBody(value)
